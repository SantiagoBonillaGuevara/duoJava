      ...
      <main className="leaderboard-main">
        <div className="leaderboard-content-wrapper">
          ...
          <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-4">
            <div>
              <p className="text-[#6324eb] font-bold text-xs uppercase tracking-widest mb-1">Liga de Diamante</p>
              <h1 className="text-3xl font-black text-slate-900 dark:text-white">Los Mejores Compiladores</h1>
            </div>
            <div className="flex items-center gap-4">
               <div className="flex items-center gap-2 bg-[#6324eb]/10 text-[#6324eb] px-4 py-2 rounded-full font-bold text-sm">
                  <span className="material-symbols-outlined filled-icon text-lg">leaderboard</span>
                  Histórico
                </div>
            </div>
          </header>
              ...
              <section className="podium-section !px-0">
                <div className="podium-container">
                  {podiumItems.map((item, index) => {
                    ...
                    let baseHeight = isFirst ? "h-40" : isSecond ? "h-24" : "h-20";
                    ...
                    return (
                      <div key={item.userId} className={`podium-item group ${topClass}`}>
                        {isFirst && (
                          <span className="material-symbols-outlined text-[#FFD700] mb-2 animate-bounce filled-icon">workspace_premium</span>
                        )}
                        <div className={`podium-avatar-wrapper group-hover:scale-105 ${shadowClass}`}>
                          <div className={`${sizeClass} podium-avatar`} style={{ borderColor: rankColor }}>
                            {item.avatarUrl ? (
                          ...
                          <div className="podium-rank-badge" style={{ backgroundColor: rankColor, color: "#1e293b" }}>
                            {item.rank}
                          </div>
                        </div>
                        <p className={`font-bold text-slate-900 dark:text-white truncate max-w-full px-2 ${isFirst ? 'text-xl' : 'text-base'}`}>
                          {item.displayName}
                        </p>
                        <p className="text-xs font-bold text-[#6324eb] uppercase tracking-wider">{item.xp.toLocaleString()} XP</p>
                        <div 
                        ...
                        >
                          <span className="text-[10px] font-black tracking-[0.2em] mb-4" style={{ color: rankColor }}>{label}</span>
                        </div>
                      </div>
                    );
                  })}
                    ...
                    <div className="z-10 flex items-center gap-6">
                      ...
                      <div className="w-12 h-12 rounded-full border-2 border-[#6324eb] overflow-hidden bg-white dark:bg-slate-800">
                        {currentUserRank.avatarUrl ? (
                          <img src={currentUserRank.avatarUrl} alt={currentUserRank.displayName} className="w-full h-full object-cover" />
                        ) : (
                      ...
                      <div>
                        <p className="font-bold text-slate-900 dark:text-white">{currentUserRank.displayName} (Tú)</p>
                        <p className="text-[10px] font-black text-[#6324eb] uppercase tracking-widest">
                          NIVEL {currentUserRank.levelNumber} • {currentUserRank.levelTitle}
                        </p>
                      </div>
                    </div>
                    <div className="z-10 bg-white dark:bg-slate-800/50 px-6 py-3 rounded-2xl border border-[#6324eb]/20 ml-auto text-center">
                      <p className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Tu XP Total</p>
                      <p className="font-black text-xl text-[#6324eb]">{currentUserRank.xp.toLocaleString()}</p>
                    </div>
                  </div>
                </section>
              )}
              ...
              <section className="global-table-container !px-0 !pb-0 min-h-[400px]">
                <div className="leaderboard-table-wrapper !max-w-none">
                  <div className="table-header">
                    <span className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Rango</span>
                    <span className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Usuario</span>
                    <span className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Insignia</span>
                    <span className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest text-right">Total XP</span>
                  </div>
                  ...
                  <div className="overflow-y-visible">
                    {others.map((item) => (
                      <div key={item.userId} className="table-row group">
                        ...
                        <span className="font-black text-slate-900 dark:text-white text-right">{item.xp.toLocaleString()}</span>
                      </div>
                    ))}
                    {others.length === 0 && (
